...
class Wandb:
    ...
    @staticmethod
    def launch(cfg, conf_filepath, launch: bool):
        if launch:
            import wandb
            ...
            # shutil.copyfile(
            #     os.path.join(os.getcwd(), "./unet_reg.yaml"), os.path.join(os.getcwd(), ".hydra/hydra-config.yaml")
            # )
            filename = os.path.join(os.getcwd(), conf_filepath)
            ff = os.path.basename(filename)

            shutil.copyfile(filename, 
                            os.path.join(wandb.run.dir, ff))
            wandb.save(os.path.join(os.getcwd(), ff))
